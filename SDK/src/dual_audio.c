...
DUAL_Result DUAL_Sound_LoadFromFile(DUAL_AudioManager* audio, DUAL_ResourceManager* resources, const char* chemin_fichier, DUAL_Sound** out_sound) {
    ...
    DUAL_ResourceHandle* sound_handle = NULL;
    if (resources) {
        DUAL_Result res = DUAL_ResourceManager_Track(resources, DUAL_MEMORY_RAM, DUAL_RESOURCE_SOUND, (uint64_t)sound->dataSize, chemin_fichier, &sound_handle);
        if (res != DUAL_OK) {
    ...
    if (!sound->pData) {
        ma_decoder_uninit(&decoder);
        if (resources && sound_handle) DUAL_ResourceManager_Untrack(resources, sound_handle);
        free(sound);
        return -1;
    }
    ...
    if (resources && sound_handle) {
        ...
        DUAL_Internal_ResourceHandle_SetCallback(sound_handle, sound, (void(*)(DUAL_ResourceManager*, void*))DUAL_Sound_Destroy);
    }
...
void DUAL_Sound_Destroy(DUAL_ResourceManager* resources, DUAL_Sound* sound) {
    if (!sound) return;
    if (resources && sound->handle) {
        DUAL_ResourceManager_Untrack(resources, sound->handle);
    }
...
DUAL_Result DUAL_Music_OpenFromFile(DUAL_AudioManager* audio, DUAL_ResourceManager* resources, const char* chemin_fichier, DUAL_Music** out_music) {
    ...
    DUAL_ResourceHandle* music_handle = NULL;
    if (resources) {
        DUAL_Result res = DUAL_ResourceManager_Track(resources, DUAL_MEMORY_RAM, DUAL_RESOURCE_MUSIC, (uint64_t)music->streamBufferSize, chemin_fichier, &music_handle);
        if (res != DUAL_OK) {
            free(music);
            return res;
        }
    ...
    if (ma_sound_init_from_file(&audio->engine, chemin_fichier, MA_SOUND_FLAG_STREAM, &audio->musicGroup, NULL, &music->sound) != MA_SUCCESS) {
        if (resources && music_handle) DUAL_ResourceManager_Untrack(resources, music_handle);
        free(music);
        return -1;
    }

    music->handle = music_handle;

    if (resources && music_handle) {
        extern void DUAL_Internal_ResourceHandle_SetCallback(DUAL_ResourceHandle* handle, void* ptr, void (*cb)(DUAL_ResourceManager*, void*));
        DUAL_Internal_ResourceHandle_SetCallback(music_handle, music, (void(*)(DUAL_ResourceManager*, void*))DUAL_Music_Close);
    }
...
void DUAL_Music_Close(DUAL_ResourceManager* resources, DUAL_Music* music) {
    if (!music) return;
    if (resources && music->handle) {
        DUAL_ResourceManager_Untrack(resources, music->handle);
    }
